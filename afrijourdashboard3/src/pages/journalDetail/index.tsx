...
export default function JournalDetail() {
    ...
    const fetchJournal = async () => {
      try {
        const response = await fetch(
          `https://backend.afrikajournals.org/journal_api/api/journals/${journalId}/`
        )
        ...
        if (data.volumes) {
          data.volumes.sort((a, b) => {
            ...
            if (volA.volumeNo !== volB.volumeNo)
              return volB.volumeNo - volA.volumeNo
  ...
  const getImageUrl = (imagePath: string) =>
    imagePath.startsWith('/')
      ? `https://backend.afrikajournals.org${imagePath}`
      : `https://backend.afrikajournals.org/${imagePath}`
      ...
      <Layout.Body>
        <div className='mx-auto max-w-7xl px-4 py-8'>
          <div className='grid gap-8'>
                    ...
                    {journal.image ? (
                      <>
                        <img
                          ...
                          className={`object-full h-full w-full transition-opacity duration-300 ${
                            imageStatus === 'loaded'
                              ? 'opacity-100'
                              : 'opacity-0'
                          }`}
                      ...
                      <div className='flex aspect-[3/4] w-full items-center justify-center rounded-lg bg-gray-200'>
            ...
            <div className='rounded-xl bg-[#e3effa] p-6 shadow-lg'>
            {/* <div className='rounded-xl bg-gradient-to-r from-[#9baed1] to-[#f5f7fa] p-6 shadow-lg'> */}
            {/* <div className='rounded-xl bg-[#c8d4e5] p-6 shadow-lg'> */}

              <div className='flex flex-col gap-8 lg:flex-row'>
                {journal.image && imageStatus !== 'error' ? (
                  <div className='lg:w-1/3'>
                    <div className='relative aspect-[3/4] w-full overflow-hidden rounded-lg bg-white shadow-md'>
                      <img
                        src={getImageUrl(journal.image.image)}
