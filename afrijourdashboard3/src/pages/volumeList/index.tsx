...
//         `${BASE_URL}/journal_api/api/user-volumes/${id}/`,
//         {
//           method: 'DELETE',
//           headers: {
...
//       } else {
//         console.error('Failed to delete volume', response.status)
...
//           setVolumes(data) // The API returns an array directly
//         } else {
//           setError(`Failed to fetch volumes: ${response.status}`)
//         }
//       } catch (error) {
//         console.error('Error fetching volumes:', error)
//         setError('Error fetching volumes')
//       } finally {
//         setLoading(false)
//       }
//     }
...
interface Volume {
  id: number
  journal: string
  journal_id: number // Required for the PUT payload
  volume_number: number
  issue_number: number
  year: number
  article_count: number
...
const index = () => {
  ...
  //   try {
  //     // API endpoint: http://localhost:8000/journal_api/api/volume/{id}/
  //     const response = await fetch(
  //       `${BASE_URL}/journal_api/api/volume/${editingVolume.id}/`,
  //       {
  //         method: 'PUT',
  //         headers: {
  //           Authorization: `Bearer ${token}`,
  ...
  //         },
  //         body: JSON.stringify({
  //           journal_id: editingVolume.journal_id,
  //           volume_number: editingVolume.volume_number,
  //           issue_number: editingVolume.issue_number,
  ...
  //       setVolumes((prev) =>
  //         prev.map((v) => (v.id === updatedData.id ? updatedData : v))
  //       )
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/journal_api/api/volume/${editingVolume.id}/`,
        {
          ...
          body: JSON.stringify({
            // Start from journal_id and exclude the main 'id'
            journal_id: editingVolume.journal_id || 6016, 
            volume_number: editingVolume.volume_number,
            issue_number: editingVolume.issue_number,
            year: editingVolume.year,
          }),
    ...
    <Layout>
      <Layout.Body>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl animate-in fade-in zoom-in duration-200">
              <div className="flex items-center justify-between border-b pb-3 mb-4">
                <h3 className="text-xl font-bold text-gray-800">Edit Volume</h3>
              ...
              <form onSubmit={handleUpdate} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingVolume({ ...editingVolume, volume_number: parseInt(e.target.value) })}
                    required
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    className="w-full rounded-md border border-gray-300 p-2 focus:ring-2 focus:ring-blue-500 outline-none"
                    value={editingVolume.issue_number}
                    onChange={(e) => setEditingVolume({ ...editingVolume, issue_number: parseInt(e.target.value) })}
                    required
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingVolume({ ...editingVolume, year: parseInt(e.target.value) })}
                    required
                  />
                </div>
                <div className="flex gap-3 pt-4">
                  <button type="submit" className="flex-1 rounded-md bg-blue-600 py-2 text-white font-semibold hover:bg-blue-700 transition-colors">
                    Save Changes
                  </button>
                  <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 rounded-md bg-gray-100 py-2 text-gray-700 font-semibold hover:bg-gray-200 transition-colors">
                    Cancel
                  </button>
                </div>
              </form>
        ...
        <div className='grid gap-6 p-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
          {loading ? (
            <div className='col-span-full flex h-[80vh] w-full items-center justify-center'>
              <p className='text-center text-lg text-gray-600'>Loading volumes...</p>
            </div>
          ) : error ? (
            <div className='col-span-full flex h-[80vh] w-full items-center justify-center'>
              <p className='text-center text-lg text-red-500'>{error}</p>
            ...
            volumes.map((volume) => (
              ...
              >
                ...
                {/* Volume Info */}
                <CardHeader className='mb-3 space-y-1 text-center'>
                  <CardTitle className='truncate text-base font-semibold leading-snug text-gray-800 px-4'>
                    {volume.journal}
                  </CardTitle>
                  <CardDescription className='text-xs text-gray-500'>
                    Volume {volume.volume_number} • Issue {volume.issue_number}
                ...
                <CardContent className='space-y-2 text-center text-sm text-gray-700'>
                  <p>
                    <span className='font-medium text-gray-900'>Year:</span> {volume.year}
                  </p>
                  <p>
                    <span className='font-medium text-gray-900'>Articles:</span> {volume.article_count}
                  </p>
                </CardContent>
              </Card>
            ))
            ...
            <div className='col-span-full flex h-[80vh] w-full flex-col items-center justify-center'>
              <p className='text-center text-lg text-gray-600'>No volume Added.</p>
            </div>
          )}
        </div>
      </Layout.Body>
